  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message =
          body?.error?.message ??
          (response.status === 401 ? "Giriş bilgileri doğrulanamadı." : "Giriş yapılamadı.");
      ...
      setState({ status: "success", message: "Başarıyla giriş yaptınız. Yönlendiriliyorsunuz..." });
      ...
      <Card className="shadow-lg">
        <CardHeader className="space-y-2 text-center">
          ...
          <CardDescription className="text-sm text-muted-foreground">
            Giriş yaparak ölçümlerinizi, bildirimlerinizi ve hedeflerinizi takip edin.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="rounded-md border border-dashed border-border bg-muted/40 p-3 text-xs text-muted-foreground">
              Bu form hCaptcha ile korunmaktadır. İnsan olduğunuzu doğrulamak için aşağıdaki bileşeni tamamlayın.
            </div>
            ...
            <Button className="w-full" type="submit" disabled={state.status === "submitting"}>
              {state.status === "submitting" ? "Giriş yapılıyor..." : "Giriş yap"}
            </Button>
          </form>
        </CardContent>
